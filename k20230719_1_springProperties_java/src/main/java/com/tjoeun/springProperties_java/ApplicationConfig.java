...
//	@Configuration 어노테이션을 붙여서 만든 클래스에서 @Bean 어노테이션을 붙여 bean을 생성하는 메소드가 
//	실행될 때 자동으로 실행되는 메소드로 properties 파일의 데이터를 읽어서 필드를 초기화 할 준비를 하는 메소드
//  => 반드시 정적(static) 메소드로 만들어야 한다.
//  main 메소드와 거의 동시에 같이 시작하는 메소드
	@Bean
...
//	properties 파일의 내용이 저장돼서 리턴되는 리턴 타입이 PropertySourcesPlaceholderConfigurer인 메소드가
//	리턴하는 내용 중 필드에 저장할 데이터의 key를 @Value 어노테이션 ${}를 사용해서 @Value 어노테이션이
//	설정된 필드에 저장한다.
	@Value("${admin.id}")

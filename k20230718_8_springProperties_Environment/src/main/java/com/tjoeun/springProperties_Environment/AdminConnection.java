...
//	EnvironmentAware 인터페이스를 구현하면 사용할 수 있고 EnvironmentAware 인터페이스가 구현된 클래스의
//	bean이 생성(refresh() 메소드가 실행)된 후 자동으로 실행되는 메소드
//	setEnvironment 메소드의 인수인 DI 컨테이너 환경 설정 정보를 기억하는 Environment 인터페이스 타입의
//  객체 environment에 스프링이 알아서 EnvironmentAware 인터페이스가 구현된 클래스의 bean이 생성되는 순간
//	DI 컨테이너의 환경 설정 정보를 넘겨준다 => properties 파일의 정보가 넘어온다.
	@Override
	public void setEnvironment(Environment environment) {
		...
		// Environment 인터페이스 객체로 넘어온 DI 컨테이너의 환경 설정 정보를 AdminConnection 클래스에서

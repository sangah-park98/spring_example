...
//	around
//	1. around AOP 메소드는 핵심 기능이 실행되고 난 후 리턴되는 데이터 타입을 예측할 수 없으므로 모든 데이터
//	   타입을 포함할 수 있는 자바의 최상위 클래스인 Object로 지정한다. 	
//	2. around AOP 메소드의 인수인 ProceedingJoinPoint 인터페이스 타입의 객체로 스프링이 자동으로 실행할
//     메소드를 넘겨준다.
//		 => 반드시 인수로 ProceedingJoinPoint 인터페이스 타입의 객체를 사용한다.
//		 => pom.xml 파일의 <scope>runtime</scope> 는 주석처리 해야 실행이 가능하다.
//	3. around AOP 메소드는 try ~ finally 형태로 실행하며 catch는 throw Throwable로 처리한다.
	public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			...
			// ProceedingJoinPoint 인터페이스 객체로 넘어온 핵심 기능을 실행하고 실행 결과를 Object 클래스
			// 객체에 저장해서 리턴시킨다.
			Thread.sleep(2000);

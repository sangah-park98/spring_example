...
//	게시판의 대문은	글목록을 얻어와서 화면에 보여주는 list.jsp를 글 목록을 얻어와서 보여준다.
	@RequestMapping("/")
...
//	글 입력폼(insert.jsp)를 호출하는 메소드	
	...
	/*
//	1. 글 입력폼에 입력된 내용을 테이블에 저장하는 메소드 => HttpServletRequest 인터페이스 객체 사용
	@RequestMapping("/insertOK")
	public String insertOK(HttpServletRequest request, Model model) {
		logger.info("insertOK() 메소드 실행 -  HttpServletRequest 인터페이스 객체 사용"); 
	...
	*/
	...
	/*
//	2. 글 입력폼에 입력된 내용을 테이블에 저장하는 메소드 => 커맨드 객체 사용
	@RequestMapping("/insertOK")
...
//	3. 글 입력폼에 입력된 내용을 테이블에 저장하는 메소드 => Model 인터페이스 객체 사용
	@RequestMapping("/insertOK")
	public String insertOK(HttpServletRequest request, Model model, MvcBoardVO mvcBoardVO) {
		logger.info("insertOK() 메소드 실행 -  Model 인터페이스 객체 사용"); 
		
		// insert.jsp에서 입력한 데이터가 저장된 HttpServletRequest 인터페이스 객체를 Model 인터페이스 객체에
		// 저장한다.
...
//	브라우저에 출력할 1페이지 분량의 글 목록을 얻어오고 1페이지 분량의 글 목록을 출력하는 메소드
	@RequestMapping("/list")
	public String insertOK(HttpServletRequest request, Model model) {
...
//	조회수를 증가시키는 메소드	
	...
	public String increment(HttpServletRequest request, Model model) {
		...
		// 컨트롤러에 "/increment"로 요청하는 페이지에서 넘어오는 클릭한 글의 글번호와 그 글이 있는
...
//	조회수를 증가시킨 글 1건을 얻어오는 메소드	
	...
	public String contentView(HttpServletRequest request, Model model) {
		...
		// 컨트롤러에 "/contentView"로 요청하는 페이지에서 넘어오는 조회수를 증가한 글의 글번호와
		// 그 글이 있는 페이지 번호가 저장된 HttpServletRequest 인터페이스 객체를 Model 인터페이스 객체에 저장한다.
		model.addAttribute("request", request);
...
// 	글 1건을 수정하는 메소드
	...
	public String update(HttpServletRequest request, Model model) {
		...
		// 컨트롤러에 "/update"로 요청하는 페이지에서 넘어오는 수정할 글의 글번호와 데이터(제목, 내용),
		// 수정 후 돌아갈 페이지 번호가 저장된 HttpServletRequest 인터페이스 객체를 Model 인터페이스 객체에 저장한다. 
		model.addAttribute("request", request);
...
// 	글 1건을 삭제하는 메소드
	...
	public String delete(HttpServletRequest request, Model model) {
		...
		// 컨트롤러에 "/delete"로 요청하는 페이지에서 넘어오는 수정할 글의 글번호와 데이터(제목, 내용),
		// 수정 후 돌아갈 페이지 번호가 저장된 HttpServletRequest 인터페이스 객체를 Model 인터페이스 객체에 저장한다. 
		model.addAttribute("request", request);
...
//	답글을 위치에 맞게 삽입하는 메소드
	...
	public String replyInsert(HttpServletRequest request, Model model) {
		...
		// 컨트롤러에 "/replyInsert"로 요청하는 페이지에서 넘어오는 원본 글번호, 글그룹, 글레벨, 같은 
		// 글그룹에서 글 출력순서, 답글 작성자 이름, 답글 제목, 답글 내용, 답글을 저장하고 돌아갈
		// 페이지 번호가 HttpServletRequest 인터페이스 객체를 Model 인터페이스 객체에 저장한다.
		model.addAttribute("request", request);

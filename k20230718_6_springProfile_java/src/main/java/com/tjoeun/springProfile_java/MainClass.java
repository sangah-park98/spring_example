...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		// profile이 설정된 xml 파일의 bean을 읽어오기 위해서는 DI 컨테이너를 먼저 만든 후 읽어올 bean의 profile을 
		// 지정한 다음 해당 profile이 지정된 bean을 load 시켜야 한다.
		// GenericXmlApplicationContext ctx = new GenericXmlApplicationContext(); // xml
		
		// profile이 설정된 java 파일의 bean을 읽어오기 위해서는 DI 컨테이너를 먼저 만든 후 읽어올 bean의 profile을 
		// 지정한 다음 해당 profile이 지정된 bean을 load 시켜야 한다.
		...
		// load() 메소드를 사용했지만 AnnotationConfigApplicationContext 클래스로 java 파일에서 @Profile 어노테이션을 
		// 붙여서 설정한 bean 설정 정보를 넣어주려면 
